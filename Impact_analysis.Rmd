...
# Source custom function to plot lines 
suppressMessages(
  source('script/toplineplot.R', 
         encoding = "UTF-8") 
)
...
The output above provides elements for the analysis of water access based on the regression analysis with PSM-matched data.
...
- **Not_applicable (3)**: This category is less relevant in this context but included for completeness.

The project intervention's coefficient is negative and statistically significant, indicating that, on average, households that received the treatment are slightly less likely to have access to at least 15 litres of water per person per day compared to those that did not receive the treatment. Possible reasons include prioritisation of the most vulnerable areas, for example. Initial implementation challenges may also have an impact. During the early stages of the project, there might have been logistical or operational challenges that temporarily hindered water access improvements. In some cases, the intervention might have focused on other urgent needs or longer-term infrastructure projects that did not immediately improve water access but could have other long-term benefits. Measurement timing may also play a role since the impact assessment might have occurred during a period of transition or adjustment, where the full benefits of the intervention had not yet been realised.


**Important Covariates**:

1. **Water Source Location (wash.location)**:
   - The location of the water source is a critical factor. Households with better proximity to water sources have significantly improved access (p < 0.001).

2. **Basic Water Needs (wash.basic_needs)**:
   - Households that meet their basic water needs are more likely to have sufficient water access. This factor shows a significant positive association (p < 0.001).

3. **Fetching Time (wash.fetchingtime)**:
   - Shorter water fetching times are associated with better water access, highlighting the importance of reducing the distance or time required to collect water (p = 0.001859).

4. **Availability of Soap (wash.nfi_soap)**:
   - The availability of non-food items such as soap also plays a role in water access, with mixed effects. The linear term shows a marginally significant positive association (p = 0.053288), while the quadratic term is negatively associated (p = 0.027598).

5. **Year 1 Implementation (year_1Yes)**:
   - The implementation phase also significantly affects water access, with a negative coefficient indicating that initial implementation stages might have faced challenges (p < 0.001).
...
1. **Improve Proximity to Water Sources**: Reduce the distance between households and water sources through infrastructure projects that bring water sources closer to residential areas.

2. **Enhance Distribution of Hygiene Supplies**: Ensure the availability of non-food items such as soap to improve overall water access. Prioritise hygiene promotion programs.

3. **Address Basic Water Needs**: Secure the basic water needs of households by providing sufficient water for daily use.

4. **Reduce Water Fetching Times**: Implement measures to decrease the time required to collect water, such as closer water points and efficient water distribution systems.

5. **Monitor Implementation Phases**: Continuously monitor and evaluate the impact of different implementation phases on water access to optimise the timing and sequencing of interventions.
...
A DiD model was employed to assess the impact of the project intervention on households' access to at least 15 liters of water per person per day (`wash.percapita`). This model includes the interaction term between the implementation period and the post-intervention period, along with various covariates identified as significant in previous analyses.
...
1. **Intervention Effect**:
   - The coefficients for the treatment periods (`implementationYear 1 to 3` and `implementationYear 2 to 3 alone`) are negative and statistically significant, suggesting that during these periods, there were challenges in ensuring water access, compared to households in areas not covered by the project. Specifically:
     - `implementationYear 1 to 3`: Estimate = -0.334804, p-value = 0.044942
     - `implementationYear 2 to 3 alone`: Estimate = -0.105108, p-value = 0.015416

2. **Significant Covariates**:
   - **Water Source Location** (`wash.location.L`): A positive and statistically significant association with water access, indicating that closer proximity to water sources improves water availability. (Estimate = 0.677680, p-value = 0.060653)
   - **Fetching Time** (`wash.fetchingtime.L`): A positive association, suggesting shorter fetching times are beneficial for water access. (Estimate = 0.097943, p-value = 0.042629)
   - **Availability of Soap** (`wash.nfi_soap.Q`): A negative association, indicating that households with sufficient soap might face less water access issues. (Estimate = -0.107781, p-value = 0.026898)

**Potential Policy Recommendations:**

1. **Enhance Proximity to Water Sources**:
...
2. **Improve Water Fetching Efficiency**:
...
3. **Promote Hygiene Supplies**:
   - **Distribution of Soap**: Ensure the continuous availability of hygiene supplies, such as soap, which has shown to have a mixed but generally positive impact on water access.
